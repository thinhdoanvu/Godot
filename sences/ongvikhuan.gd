...
onready var canhbao_frame = get_parent().get_node("error2")
onready var canhbao_error = get_parent().get_node("lbl_error2")
...
func _process(delta):
	...
	if quecay.isclick && error_khukhuan_l2:#khu khuan lan 2 sau khi ve xong vung 1
		...
		hide_quecay()
	elif quecay.isclick && error_khuanlac_l2:#lay khuan la thanh cong lan thu 2
		vikhuan.visible = true
		quecay.iskhukhuan = false
		vikhuan.scale = Vector2(1.4,1.5)
		hide_quecay()
	#Canh bao chua khu khuan
	elif quecay.isclick && error_khukhuan:
		canhbao_frame.visible = true
		canhbao_error.visible = true
		canhbao_button.visible = true
	...
	elif quecay.isclick && error_khuanlac:
		canhbao_frame.visible = true
		canhbao_error.visible = true
		canhbao_button.visible = true
		canhbao_error.text = "Bạn cần chọn khuẩn lạc bên dưới (vùng dung dịch có màu vàng)"
		hide_quecay()
	elif quecay.isclick && pick_khuanlac:#pick khuan lac thanh cong
		vikhuan.visible = true
		quecay.iskhukhuan = false

	#else:#KHI BUONG CHUOT THI DONG THONG BAO
