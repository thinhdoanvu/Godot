...
func _on_but_bunsen_button_up():
	...
	#kiem tra xem da khu khuan ong vi khuan chua:
	#Neu chua:
	if not get_parent().get_node("Path2D/PathFollow2D/nap/ar_open").visible:
		get_parent().get_node("anim_khukhuanvk").play("run")
